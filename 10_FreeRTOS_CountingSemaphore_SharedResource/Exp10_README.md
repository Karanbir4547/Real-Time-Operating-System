...
> A counting semaphore is a gated pool with multiple simultaneous entry slots.
...
```
 ...
 |  Token Pool                      |
 |  +------+  +------+             |
 |  | [T]  |  | [T]  |             |  <- 2 tokens available at start
 |  +------+  +------+             |
 +----------------------------------+
...
| Counting Sem.    | N (custom) | Resource pools   | No (N at a time)   |
+------------------+------------+------------------+--------------------+
```
...
This is **intentional, not a defect** -- it perfectly illustrates that counting semaphores control how many tasks may simultaneously access a resource, but not the order or atomicity of those accesses.
...
**Tasks and Queues tab -- Three Tasks:**
...
**Timers and Semaphores tab -- Counting Semaphore:**
...
With count=2, two tasks hold the semaphore at the same time:
...
| Replace counting semaphore with a mutex | True mutual exclusion -- clean sequential output, one task at a time |
...
1. What does "thread-safe" mean -- and why does a counting semaphore with count greater than 1 not fully guarantee it for a shared ITM or UART port?
2. Implement a fast producer and slow consumer using the counting semaphore -- what happens when the count reaches zero?
...
Three tasks competed for a shared resource (the ITM trace port) governed by a Counting Semaphore initialized to count = 2. The SWV output clearly showed task interleaving -- two tasks wrote simultaneously, producing irregular mixed character sequences. This demonstrated that counting semaphores control resource availability rather than atomicity, drawing a clear distinction from mutexes in practical RTOS system design.
