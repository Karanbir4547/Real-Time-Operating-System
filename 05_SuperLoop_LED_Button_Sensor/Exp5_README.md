...
> No RTOS. No waiting. Just an intelligent use of a millisecond timestamp.

---

## Objective

Implement a **non-blocking super loop** that runs three independent periodic tasks concurrently — LED toggling, button polling, and IR sensor sampling — all on a single CPU core, without any `HAL_Delay()` blocking calls.
...
**Core idea:** Each task checks whether its scheduled interval has passed — none of them waits for it.
...
- Resolution: 12-bit (default) -> raw values range 0 to 4095
...
while (1) {
    ...
    // No HAL_Delay() here -- that would halt all three tasks!
}
/* USER CODE END WHILE */
```
...
| **Super Loop (this experiment)** | Yes | Yes | Yes | 0% (loop runs freely) |
...
4. Resume and watch variables update in real time
```
...
| Add a 4th task (e.g., buzzer) | Declare `buzzer_counter_ms` and increment it alongside the others |
...
2. What role do the three software counters play in producing independent task rates?
...
5. Under what conditions would a super loop be insufficient, requiring a full RTOS instead?

---

## Result

The non-blocking super loop successfully ran three independent periodic tasks in parallel without stalling, demonstrating that `HAL_GetTick()`-based software counters are a lightweight and effective alternative to RTOS task scheduling for resource-constrained embedded applications.
